...
- **Tres ajustes al prompt SYSTEM del generador** (commit `c3ffefd`, [`src/generate.py`](src/generate.py)):
  - Refina **"DECLARA LA NATURALEZA DE CADA CIFRA"**: restringe `(dato oficial)` a URL institucional (BOIB, web del organismo, nota institucional, resolución administrativa). Añade `(dato oficial vía medio)` para cuando el medio presente la cifra como oficial pero la URL sea del propio medio sin primaria localizada en el input. Hipótesis de impacto: rebaja la disputa Haiku↔Sonnet al desambiguar la categoría intermedia que cada modelo interpretaba a su modo.
  - Nueva **"CONSOLIDACIÓN DE ACTORES"**: si dos nombres de actor referencian al mismo organismo, o si dos propuestas derivan de la misma declaración pública (mismo evento, fecha, portavoz), se unifican en una sola entrada del Mapa y una sola propuesta. Si son órganos institucionalmente distintos, declarar la diferencia en una frase del cuerpo. Origen: Colegio de Arquitectos de Baleares y Colegio de Arquitectos de Ibiza desdoblados en dos propuestas a partir del mismo evento de prensa del 12 de mayo.
  - Nueva **"CIERRE DE EVENTOS DE LA CRONOLOGÍA ACTUAL"**: si la cronología sitúa un evento con resultado esperado (pleno parlamentario, votación, comparecencia) cuyo desenlace no aparece en el input clasificado, añadir la coletilla *«sin resultado publicado al cierre»* en lugar de omitirlo. Origen: pleno del Parlament del domingo 17 con vivienda en el orden del día, sin resultado recogido en la edición W21, leído por el auditor como laguna de cobertura.
- **Una mirada antes de añadir.** Las tres reglas se contrastaron contra las existentes (regla 5 de gestión documental, [D24](DECISIONES.md)): la primera refina la regla previa de naturaleza de cifra sin solape con `FUENTE AGREGADA ETIQUETADA` (que cubre agregadores como Google News, MSN); la segunda es ortogonal a `DEDUPLICACIÓN` (que opera sobre objetivo + actor_type + horizon de propuestas, no sobre identidad de actor); la tercera complementa `CIERRE EXPLÍCITO DE A VIGILAR` (que cierra items de la edición anterior, no eventos de la cronología actual). Ninguna existente absorbía las nuevas.
- **Warnings en seguimiento de W21** (registrados en [`APRENDIZAJES.md`](APRENDIZAJES.md), no ascienden a regla esta semana): (a) el caso del alcalde no cita marco legal específico de la ley balear de vivienda protegida — el generador no puede invocar reglamentación fuera del input, queda en seguimiento; (b) ausencia de voz de propietarios e inmobiliario privado pese a noticias de alquiler turístico ilegal — segunda aparición del patrón "bloque ausente" (W19 sindicato/patronal, W21 propietarios/inmobiliario); si reaparece en W22-W23 asciende a sugerencia con propuesta concreta.
- **Validación.** Las tres reglas se prueban empíricamente en la edición W22 (próximo lunes 2026-05-25). Si la tasa de disputa cae al rango sano y trazabilidad sube a ≥7, regla validada. Si no, abrir sesión específica.
...
- **Auditoría comparativa.** Revisión de los 9 proyectos restantes del editor (wallabot, monesma, panel, kangoo-fantasma, notas, actualidad-legal, landings-pymes, model-master, gear-master) + pelícano (proyecto reciente en `varios/pelicano/`, no registrado todavía en `~/.claude/STATE.md`). **Todos trabajan directo a `main`**, sin PR. Todos los CLAUDE.md dicen lo mismo: "un commit por cambio lógico, push a `main` después de cada commit". radar-ibiza era el único con flujo PR + squash + acumulación de ramas `claude/*` huérfanas en GitHub. Casos extremos detectados de paso: monesma (10+ ramas vivas), panel (5+ ramas vivas), pelícano (1 rama viva tras su único PR squash-mergeado) — pendientes globales para limpieza en sesión específica.
...
- **Lo que NO toca esta entrada.** Limpieza de ramas `claude/*` huérfanas en monesma, panel y pelícano. Decidir destino de la rama actual `claude/keen-bhaskara-3a8c75` una vez cerrada la sesión. Registrar pelícano en `~/.claude/STATE.md`. Apuntado como pendiente sin afectar al alcance de D43.
...
- **Repaso del índice antes de redactar.** El índice inicial de 14 secciones tenía huecos estructurales detectados en repaso explícito al editor: canales fuera de la web (newsletter, RSS, OG, imprimible), página de actor con dimensión BOIB como anatomía propia, propuesta en cuarentena con vínculo BOIB pendiente, direccionamiento canónico de disposiciones, frontera con tiers/balance/correcciones, defaults expandido vs colapsado como principio nominado, glosario, trilingüe. El editor optó por meterlos todos. Índice final: 18 secciones.
- **Documento redactado.** [`ESTUDIO-ARQUITECTURA-DUAL.md`](ESTUDIO-ARQUITECTURA-DUAL.md) abierto con resumen ejecutivo, 6 principios duros, capas por público y por canal, decisión de direccionamiento canónico (opción C: URL propia `/normativa/{slug}` con cita técnica + enlace a PDF original de boib.caib.es), anatomías de cinco componentes (ficha de propuesta con cabecera ✅/❌/🟡, sección semanal "📜 Lo publicado", `/normativa/`, `/sin-reflejo/`, página de actor con ratio "promesas vs reflejadas"), reglas duras de estilo editorial con ampliación del glosario, frontera con sistemas existentes (tiers ≠ override automático; balance dimensión BOIB diferida con patrón 5; correcciones bajo mismo protocolo de 72h con etiqueta interna), tres escenarios reales maquetados textualmente, cinco riesgos abiertos.
- **Decisiones operativas internas del sub-estudio (no migran a DECISIONES.md hasta validar en cara real).** Cabecera con etiqueta única ✅/❌/🟡 sobre cada ficha. ❌ en gris claro y no rojo (evita lectura adversarial). 🟡 con ciclo de 30 días antes de retirarse. Sección "📜 Lo publicado" con tope de 5 disposiciones por edición + enlace al resto. Página de actor sin gráfico de barras (ratio textual). `/sin-reflejo/` distinta de `/sin-dato/` (ya existente para campos sin evaluar). Glosario único centralizado, no entradas duplicadas por página. Tope 30% de superficie editorial BOIB por edición como regla anti-drift. Texto técnico citado siempre en idioma original (CA o ES); resumen claro traducido en Fase 4.
- **Lo que NO se ha tocado en esta sesión.** Implementación. Se mantiene la disciplina del padre: arquitectura dual cerrada **antes** de tocar código + criterios objetivos del clasificador (precisión ≥85%, exhaustividad ≥80%, tasa de cuarentena <15%) cumplidos en prueba en seco antes de arrancar Movimiento 1. El sub-estudio paralelo del clasificador todavía debe cerrar §9 con esa prueba en seco.
- **Sincronización docs vivos.** [`STATUS.md`](STATUS.md) (línea de Movimiento 1 ampliada con dos sub-estudios + docs vivos en raíz a 30), [`ROADMAP.md`](ROADMAP.md) Fase 2 (sub-estudio 2 ya abierto), `DIARIO.md` (esta entrada). Sin nueva D — el sub-estudio es operativo y sus decisiones internas se convierten en D solo si validan en cara real (siguiendo el principio de "no inventar criterios artificiales para inflar" que guio la migración D28-D40).
...
- **Tres fuentes consolidadas en una.** Antes: (1) `DECISIONES.md` con D0-D27 (decisiones del 23-abr en adelante), (2) [`ESTUDIO-DISENO.md`](ESTUDIO-DISENO.md) §11 con 13 decisiones cerradas el 21-abr llamadas internamente "D1-D13 del estudio" (sin colisión con las D1-D13 ya migradas a DECISIONES.md, que tenían significado distinto), (3) [`DECISIONES-PENDIENTES.md`](DECISIONES-PENDIENTES.md) con 16 cuestiones cerradas el 20-abr. Ahora: `DECISIONES.md` con D0-D40 (las nuevas D28-D40 son las migradas del estudio) + Anexo con tabla resumen del cuestionario del 20-abr.
- **Migración del estudio (D28-D40) — formato completo.** Cada D recibe los 9 campos del formato actual (fecha original 21-abr, tema `diseno`, decisión, por qué, docs afectados, próxima revisión, criterio de revocación, estado, histórico cuando aplica). Próxima revisión y criterio de revocación se redactan con criterios honestos derivados del estudio cuando hay base; nota explícita *"ninguno explícito en el estudio original"* cuando no la hay. **No se inventan criterios artificiales para inflar.** Los matices del estudio §11 (numeración de edición, temporadas, modo oscuro Unicode, formulario opcional) se incorporan a la D correspondiente.
- **Cuestionario del 20-abr (16) — tabla resumen sin migración fila a fila.** Son hechos consumados (borrar W16-W17, newsletter sí gratis en Fase 0, diseño actual + Solar Low-Tech, modelos IA Haiku+Sonnet+Opus, etc.) cuya información operativa ya vive en `CLAUDE.md`, `ARQUITECTURA.md`, `docs/metodo.md` y demás. Migrar como decisiones append-only con criterio de revocación inventado sería ruido. La tabla resumen del anexo enlaza cada cuestión a su doc canónico actual.
- **Trazabilidad cruzada.** Tabla §11 del estudio sustituida por índice compacto (D estudio · Tema · D actual · Resumen) con enlaces a cada D28-D40 en DECISIONES.md. Cabecera nueva en `DECISIONES-PENDIENTES.md` que apunta al anexo. Regla 4 del propio `DECISIONES.md` actualizada: ya no es "migración pendiente", es "completada el 2026-05-07".
- **Detalle especial — D28 (era D1 del estudio, wordmark V2 Split).** La decisión migrada refleja el estado **actual** (`vivienda_ibiza`) con sección "Histórico" que documenta el orden original (`ibiza_vivienda`) y la revisión por [D25](DECISIONES.md) del 2026-05-07. Trazabilidad clara, sin contradicción.
- **Detalle especial — D33 (era D6 del estudio, ciclo de propuesta).** La decisión migra los 8 estados nominales y queda **concretada por [D27](DECISIONES.md)** sobre cómo se visualiza la barra. La deuda paralela del esquema `state` del pipeline (que tiene `en_movimiento` y `pendiente_resolucion_judicial`) se apunta en D33 sin resolver — sigue siendo trabajo de sesión específica al reanudar el bloque B en Jekyll.
- **Lo que NO se ha tocado.** Sección "Matices registrados" del estudio §11 (matices de numeración de edición, temporadas, modo oscuro, formulario) — los matices se incorporan también a las D nuevas pero el bloque histórico narrativo del estudio se mantiene tal cual. Sección §11bis "Opciones desechadas" del estudio — referencia histórica del rango de alternativas valoradas, queda tal cual. Memoria auto del proyecto. Entradas históricas del DIARIO. Memoria `private/`. Código.
- **Volumen.** `DECISIONES.md` pasa de 27 decisiones + cabecera (≈340 líneas tras D27) a 40 decisiones + cabecera + anexo (≈540 líneas tras la migración). Manejable, sigue siendo navegable. Próxima revisión natural cuando dispare la auditoría sistémica de [D24](DECISIONES.md): ahora hay D25-D27 + D28-D40 = 16 decisiones nuevas desde la auditoría bootstrap del 2026-05-05; el umbral de 5 ya está superado, aviso por Telegram dispararía el próximo lunes vía `decisions_watch.py`. Apunte para esa auditoría: que las 13 D28-D40 son migración formal, no decisiones nuevas en sentido estricto, y la valoración de "complejidad acumulada" debería tenerlo en cuenta.
...
- **Pregunta 3 cerrada — barra de progreso.** Decidida opción A: barra principal lineal de 6 pasos (`Propuesta → Registrada → En debate → Aprobada → En ejecución → Implementada`) con orden canónico fijo + los dos cierres alternativos (`Descartada`, `Incumplida`) como indicador aparte cuando aplican. Registrada como [D27](DECISIONES.md#d27--barra-de-progreso-de-propuestas-camino-principal-lineal-de-6--cierres-alternativos-aparte). Hallazgo del estudio: la sección §6.5 del estudio de diseño cerrado el 21-abr ya proponía esto (6 pasos + ramas) pero la maqueta actual mete los 8 lineales con orden mezclado. La maqueta queda como mockup intermedio incoherente; el rediseño real se hará al construir el componente en Jekyll (1-2 horas, fuera de esta sesión). Deuda paralela apuntada en D27: el campo `state` del esquema del pipeline y los 8 estados del estudio **no coinciden del todo** — `en_movimiento` y `pendiente_resolucion_judicial` viven en el código pero no en el estudio; `registrada` e `incumplida` viven en el estudio pero no en el código. Resolver cuando se reanude el bloque B en Jekyll.
