...
- **DECLARA LA NATURALEZA DE CADA CIFRA** con etiqueta inline la primera vez que aparezca en el cuerpo. Opciones: `(dato oficial)` SOLO si la URL apunta a la fuente primaria institucional (BOIB, web del organismo, nota de prensa institucional, resolución administrativa); `(dato oficial vía medio)` si el medio presenta la cifra como dato oficial pero la URL es del medio y no se localiza primaria en el input — declárelo inline; `(estimación periodística)` si el propio medio la acota como aproximada o de agencia; `(orientativa)` si es rango sin fuente primaria. Ejemplos: «unos 200 trabajadores *(estimación periodística)*»; «387 viviendas protegidas *(dato oficial vía medio)*»; «25M aprobados por el Govern *(dato oficial)*». Esto aplica a señales y a propuestas por igual.
...
- **CONSOLIDACIÓN DE ACTORES**: si dos nombres de actor referencian al mismo organismo (ej. *"Colegio de Arquitectos de Baleares"* y *"Colegio de Arquitectos de Ibiza"* cuando ambas etiquetas se refieren al mismo órgano colegial), o cuando dos propuestas derivan de la misma declaración pública (mismo evento, misma fecha, mismo portavoz), unifícalos en una sola entrada del Mapa de posiciones y una sola propuesta. Si son órganos institucionalmente distintos con declaraciones diferenciadas, mantén la separación pero declara la diferencia explícitamente en una frase del cuerpo (ej. *"El Colegio de Arquitectos de Baleares y el de Ibiza son órganos colegiales distintos que se pronunciaron por separado el mismo día"*). Antes de generar dos propuestas a partir del mismo evento de prensa, verifica que las peticiones son realmente diferenciadas.
...
- **CIERRE DE EVENTOS DE LA CRONOLOGÍA ACTUAL**: si la cronología de ESTA semana sitúa un evento con fecha y resultado esperado (pleno parlamentario que vota, comparecencia, fecha límite de un trámite, votación municipal) cuyo resultado NO aparece en el input clasificado de la semana, no omitas el desenlace: añade en el bullet de Cronología o como bullet propio en "Señales detectadas" la coletilla *«sin resultado publicado al cierre»*. Aplica también si el evento se anunció en la cronología sin contraparte de resultado en señales. El objetivo es declarar la laguna en lugar de dejarla muda.
