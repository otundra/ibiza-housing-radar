...
### D24 — Freno al pozo de modificación infinita: auditoría sistémica + termómetro + convención

- **Fecha:** 2026-05-05
- **Tema:** docs
- **Decisión:** se monta un freno en tres piezas para evitar que la acumulación de reglas, dimensiones y decisiones complejifique el sistema sin auditoría real:
  - **(a) Auditoría sistémica humana** disparada cuando ocurra antes de las dos: (1) hayan pasado 3 meses desde la última, (2) se hayan abierto 5 decisiones nuevas, (3) el editor lo pida. La auditoría es una sesión específica con el asistente que produce un informe corto con cuatro apartados — contradicciones detectadas entre reglas/dimensiones/decisiones, redundancias, zombis (dimensiones del revisor con nota constante 4+ semanas, reglas del generador sin warning asociado en 4+ semanas, decisiones con próxima revisión vencida hace meses sin disparar acción), presupuesto de complejidad. El editor decide en una palabra cada candidata (consolidar / retirar / mantener); cada decisión que salga se commitea aparte citando esta auditoría.
  - **(b) Termómetro de salud sistémica** como apartado nuevo en `private/panel.md` (regenerado cada lunes por `src.panel`). Cuatro métricas orientativas: reglas duras adicionales en el prompt del generador, dimensiones del revisor, decisiones vigentes en `DECISIONES.md`, candidatas a retirar (heurística: dimensiones con nota constante durante las últimas 4 ediciones consecutivas). Visibilidad continua sin auditoría humana. El conteo no dispara aviso por sí solo, pero al cruzar umbrales obvios (ej. >25 reglas en el prompt del generador, >10 decisiones vigentes nuevas en un trimestre) el editor puede invocar la auditoría con criterio.
  - **(c) Convención "una mirada antes de añadir"** como regla 5 de gestión documental en el `CLAUDE.md` del proyecto. Cuando se proponga una regla nueva al prompt del generador, una dimensión nueva al revisor o una decisión nueva, antes de aplicar hay que listar lo que ya existe en esa misma área y comprobar si alguna puede absorber, generalizarse o retirarse. No obligatorio retirar — obligatorio mirar.
- **Por qué:** sin freno explícito, las decisiones, reglas y dimensiones se acumulan. Cada decisión se revisa por su `Próxima revisión` individual, pero nadie lee el conjunto. La sesión del 2026-05-05 destapó por casualidad la tensión de `blocks_cited` entre la regla del prompt del generador (solo proponentes) y la lectura del revisor (todos los actor_types citados); sin mirada al conjunto ese tipo de contradicción queda silenciada hasta que rompe en producción. Las tres piezas se refuerzan: la auditoría es el freno periódico humano (evita enquistamiento), el termómetro es el termómetro continuo (visibilidad sin esfuerzo), la convención es la cultura previa (presión a consolidar antes de añadir). Por sí solas serían insuficientes — la auditoría sin termómetro es ciega, el termómetro sin auditoría es ruido, la convención sin las otras dos es voluntarismo.
- **Cifras de partida (2026-05-05, tras cerrar la auditoría bootstrap):** 17 reglas duras adicionales en el prompt del generador, 6 dimensiones del revisor, 22 decisiones vigentes (incluyendo la propia D24). Estas cifras son la línea base contra la que medir crecimiento. La auditoría bootstrap se documenta en [`private/auditorias/2026-05-05-bootstrap.md`](private/auditorias/2026-05-05-bootstrap.md).
- **Reconoce la paradoja del freno:** este mecanismo añade complejidad para frenar la complejidad. Mitigado mediante envergadura mínima — una sesión cada 3 meses, un apartado nuevo en un panel ya existente, dos líneas en `CLAUDE.md`. Si tres meses muestran que el freno mismo es ruido (criterio de revocación abajo), se retira con D24 revocada.
- **Hook con `decisions_watch.py` activo desde 2026-05-05.** Persistencia en `data/auditorias.csv` (append-only: `date,last_decision_id,active_count,trigger,note`). Bootstrap registrado con fila inicial `2026-05-05,D24,21,bootstrap,…`. El módulo `src/decisions_watch.py` añade dos funciones: `check_systemic_audit_trigger()` lee la última fila y devuelve flags si ha pasado el umbral (≥90 días o ≥5 decisiones nuevas); `register_audit(note)` añade fila nueva al cerrar una auditoría. El cron del lunes dispara aviso por Telegram cuando corresponde y se silencia tras `python -m src.decisions_watch --register-audit --audit-note "…"`.
- **Docs afectados:** `DECISIONES.md` (D24 nueva), `src/panel.py` (cuatro funciones nuevas para el termómetro + apartado en la generación del panel — commit aparte), `private/panel.md` (sección nueva regenerada), `CLAUDE.md` del proyecto (regla 5 de gestión documental añadida), `DIARIO.md` (entrada nueva).
- **Próxima revisión:** primera auditoría tras 3 meses (≈2026-08-05) o tras 5 decisiones nuevas desde D24, lo que ocurra antes. Después, mismo criterio rolling.
- **Criterio de revocación:** (a) si la primera auditoría no encuentra contradicciones ni redundancias ni zombis significativos, el sistema no las está produciendo y la auditoría es ruido — bajar frecuencia a 6 meses o desmantelar; (b) si la auditoría requiere más de 90 minutos del editor por sesión, simplificar la plantilla; (c) si las decisiones que salen de auditorías no se aplican en >50% de los casos (porque el editor las descarta sistemáticamente), la auditoría no encaja con su criterio editorial real y debe rediseñarse; (d) si el termómetro no detecta zombis durante 6 meses con la heurística de "nota constante 4 ediciones", el indicador no es informativo y se sustituye o retira.
- **Estado:** vigente

### D25 — Wordmark del proyecto: orden `vivienda_ibiza` (tema antes que lugar)

- **Fecha:** 2026-05-07
- **Tema:** diseno
- **Decisión:** el wordmark oficial del proyecto pasa a `radar))vivienda_ibiza` (el tema delante, el lugar detrás). Mantiene el formato V2 Split aprobado en ID1 (las dos `))` en color terracota, el resto en JetBrains Mono). Solo se invierte el orden de las palabras separadas por guion bajo. Supera la parte del orden de la decisión informal "ID1" registrada en [REVISION-FASE-0.5.md](REVISION-FASE-0.5.md#id1--nombre-definitivo--radarivisbiza_vivienda-) (que cerró el orden contrario `ibiza_vivienda` el 21-abr).
- **Por qué:** lectura visual en cabecera real, validada al reanudar el bloque B con el prototipo abierto. El editor confirma que `vivienda_ibiza` se lee más natural en español que `ibiza_vivienda`. La motivación original del orden `ibiza_vivienda` era abrir un ecosistema futuro de marca por lugar (`radar))ibiza_turismo`, `radar))formentera_vivienda` con el lugar como raíz fija); con D25 ese ecosistema, si llega, se construirá con el tema delante (`radar))turismo_ibiza`, `radar))vivienda_formentera`). El editor declara explícitamente: *"igual más adelante cambio el nombre pero de momento así"* — este wordmark está vivo mientras no haya nombre/dominio definitivo del proyecto.
- **Docs afectados:** `CLAUDE.md` (cabecera), `STATUS.md` (sección Identidad), `ESTUDIO-DISENO.md` (cabecera + cita §4.1 + tabla §11), `ROADMAP.md` (cabecera + supuestos base + decisiones cerradas en Fase 1), `PIVOTE.md` (mención en nota), `EXPANSION-TEMATICA.md` (mención), `REVISION-FASE-0.5.md` (ID1 con nota de superación por D25 + tabla de seguimiento), `docs/acerca.md` (cara pública), memoria auto del proyecto (`nombre_proyecto.md` ya estaba con `vivienda_ibiza`, sin cambio). Entradas históricas del DIARIO con `ibiza_vivienda` no se editan (append-only).
- **Próxima revisión:** al cerrar el nombre/dominio definitivo del proyecto. El editor lo dejó abierto (*"igual más adelante cambio el nombre"*); cuando se elija dominio (`radaribiza.com` u otro) y se decida nombre definitivo del proyecto, este wordmark se reevalúa entero.
- **Criterio de revocación:** si tras la revisión visual sostenida en el bloque B (al construir las páginas en Jekyll y verlas con datos reales en flujo de lectura) el orden `vivienda_ibiza` confunde sistemáticamente al primer visitante (lo lee como nombre de archivo o módulo técnico, no como nombre de marca), volver al orden `ibiza_vivienda` o cambiar a un wordmark sin guion bajo.
- **Estado:** vigente

### D26 — Orden visual de actores en listados públicos: taxonomía pública + alfabético dentro de cada tipo

- **Fecha:** 2026-05-07
- **Tema:** diseno
- **Decisión:** en cualquier listado público que muestre varios actores (firmantes de una propuesta, mapa de posiciones, directorio `/actores/`, resumen en home, etc.), el orden visual sigue dos reglas en cascada: **(1) por tipo de actor según la taxonomía pública** publicada en `/metodo/` y `CLAUDE.md` (instituciones públicas → partidos → patronales → sindicatos → tercer sector → académicos → judicial → colectivos ciudadanos); **(2) alfabético dentro de cada tipo**. La taxonomía es la fuente del orden, no la jerarquía editorial ni la importancia política ni el orden en que el periodista los citó en la noticia original.
- **Por qué:** los listados sin orden explícito acaban con criterio implícito que el lector no entiende y que puede sospechar (por qué la patronal X aparece antes que el sindicato Y). Anclar el orden a la taxonomía publicada vuelve la regla auditable, defendible en una línea (*"seguimos la taxonomía publicada"*) y elimina sospechas de jerarquía editorial. El orden alfabético dentro de cada tipo evita que el editor decida quién va primero. La regla 3 fundacional (*"ningún actor queda excluido por filiación"*) cubre el espíritu — el orden visual también cuenta.
- **Docs afectados:** `docs/prototype/proposal.html` (reordena las 6 pildoras de la coalición W17 a la nueva regla), futuros componentes Jekyll del bloque B (cuando se construyan, aplican esta regla por defecto). Sin cambios en `/metodo/` ni en `CLAUDE.md` — la taxonomía ya estaba definida; aquí solo se anuda el orden visual a ella.
- **Próxima revisión:** al construir las páginas en Jekyll del bloque B con datos reales del backfill — primer uso real de la regla con varias coaliciones distintas.
- **Criterio de revocación:** si en uso real (a) algún tipo de actor queda sistemáticamente al final aunque sean los proponentes principales de la propuesta y eso lleva al lector a confundir quién la firma, o (b) las coaliciones más grandes (>10 firmantes) saturan visualmente el primer pliegue de la ficha de propuesta y obligan a hacer mucho scroll, sustituir por agrupado con resumen ("Coalición · 13 firmantes" con desplegable) o relajar el orden a "criterio editorial caso a caso".
- **Estado:** vigente

### D27 — Barra de progreso de propuestas: camino principal lineal de 6 + cierres alternativos aparte

- **Fecha:** 2026-05-07
- **Tema:** diseno
- **Decisión:** la barra de progreso de la ficha de propuesta muestra **el camino principal en 6 pasos lineales** con orden canónico fijo: `Propuesta → Registrada → En debate → Aprobada → En ejecución → Implementada`. Los pasos se marcan como `done` (recorridos), `current` (estado actual) o pendientes (sin marca). Si una propuesta saltó algún paso del medio (caso típico: una propuesta debatida públicamente sin haberse registrado formalmente todavía), ese paso queda visualmente vacío y el `current` aparece en su posición canónica fija. Los dos finales alternativos —`Descartada` e `Incumplida`— **no van en la barra lineal**: cuando una propuesta termina en alguno de los dos, aparece un indicador aparte tipo *"⊘ Descartada el 15 jun · fuente"* o *"⚠ Incumplida — plazo vencido sin ejecución"*. La barra principal se atenúa (gris) cuando hay cierre alternativo declarado.
- **Por qué:** (a) **Es lo que ya proponía el estudio formal §6.5** cerrado el 21-abr — el ejemplo de código del estudio mostraba 6 elementos, no 8, y un texto explícito decía que "descartada e incumplida son ramas alternativas al camino principal". La maqueta actual del prototipo se desvió: mete los 8 lineales con orden mezclado (`En debate` antes que `Registrada`). D27 alinea con el estudio, no inventa nada nuevo. (b) **Refleja la realidad institucional:** una propuesta no puede estar "Implementada" Y "Descartada" — son excluyentes. Mostrar los 8 lineales engaña al lector haciéndole creer que son pasos secuenciales. (c) **Coherente con el modelo documental:** mostrar "Incumplida" como final separado y visible es más potente como mensaje editorial que enterrarla como octavo paso pendiente. El observatorio rinde cuentas de propuestas que no se cumplen — eso merece visibilidad propia, no aparecer como "está por hacer".
- **Cómo se aplica caso por caso (orden canónico fijo):**
  - Camino canónico de éxito → 6 pasos en orden fijo, marcados `done` los pasados, `current` el actual, sin marca los pendientes.
  - Caso "saltó la fase de registro" (propuesta debatida sin expediente formal) → posición de `Registrada` queda vacía, `En debate` se muestra como `current` en su posición canónica.
  - Caso "Descartada" → barra principal en gris (no current), indicador aparte: *"⊘ Descartada el {fecha} · {fuente}"*.
  - Caso "Incumplida" → barra principal con los pasos recorridos en gris hasta donde llegó (típicamente `Aprobada` o `En ejecución`), indicador aparte: *"⚠ Incumplida — plazo vencido el {fecha} sin ejecución registrada"*.
- **Docs afectados:** `ESTUDIO-DISENO.md` §6.5 (esta decisión concreta D6 del estudio sin contradecirla — D6 ya decía 8 estados con descartada/incumplida como ramas, D27 cierra cómo se visualizan), `docs/prototype/proposal.html` (la maqueta actual no refleja D27 — queda como mockup intermedio incoherente, se rediseñará al construir el componente real en Jekyll), `docs/assets/css/main.css` futuro (componente `progress-bar` con CSS nuevo), `docs/_includes/progress-bar.html` futuro (template Jekyll del componente). El campo `state` del esquema del pipeline (`src/extract.py`) y los 8 estados del estudio **no coinciden del todo** — el código tiene `en_movimiento` y `pendiente_resolucion_judicial` que no están en el estudio; el estudio tiene `registrada` e `incumplida` que no están en el código. Es deuda de coherencia paralela que NO resuelve D27 — apuntada como tarea de sesión específica al reanudar el bloque B en Jekyll.
