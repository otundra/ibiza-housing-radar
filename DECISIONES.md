...
- **Hook con `decisions_watch.py` diferido.** El aviso automático por Telegram cuando hayan pasado 90 días desde la última auditoría requiere registrar la fecha de la última auditoría en algún sitio persistente (campo en `STATE.md` interno o tabla `data/auditorias.csv`). Se difiere a la primera auditoría real — hasta entonces el disparador es manual ("ya van 5 decisiones nuevas" o "ya van 3 meses").
